...
def main():
    adversary_fgsm = FGSM(model1, loss_fn=nn.CrossEntropyLoss(reduction="sum"), eps=0.03)
    ...
    for i in range(epoch):
        ...
        my_loss = cost / len(trainloader)
        #writer.add_scalar('test', my_loss, i)
        print('epoch: {}, Loss: {:.5f} Accuracy: {}/{} ({:.0f}%)'.format(i + 1, my_loss,correct, len(trainloader.dataset), 100. * correct / len(trainloader.dataset)))
        torch.save(model2, '/content/drive/My Drive/Colab Notebooks/DEFENSE_ADV/CIFAR/model/defense.pth')
    #writer.close()
