...
if __name__ == '__main__':
    ...
    input_size = 32
    transform_train = transforms.Compose([transforms.Resize(input_size), transforms.CenterCrop(input_size), transforms.ToTensor()])
    train_data = torchvision.datasets.ImageFolder(root='./dataset/cifar10-mini/data/imgs', transform=transform_train)
    ...
    model1.load_state_dict(torch.load('./SavedNetworkModel/CIFAR/GoogleNet/googlenet_cifar_19.pth', map_location=torch.device(device)))
    model1.to(device)
    model1.eval()
    ...
    adversary_cw = CarliniWagnerL2Attack(model1, num_classes=10, max_iterations=1000)
    ...
    for i in range(epoch):
        ...
        torch.save(model2,
                   './SavedNetworkModel/CIFAR/GoogleNet/Defense_CW/googlenet_defense_cw_cifar_{}.pth'.format(i))
