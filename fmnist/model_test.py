...
optimizer = optim.Adam([{'params': model2.parameters()},{'params': model3.parameters()},{'params': awl.parameters(), 'weight_decay': 0}],lr=0.0001)

PATH = ('/content/drive/My Drive/Colab Notebooks/DEFENSE_ADV/10月FMNIST/checkpoint/defense.pth')
checkpoint = torch.load(PATH)
model2.load_state_dict(checkpoint['com_model'])
model3.load_state_dict(checkpoint['rec_model'])
...
def tes(eps):
    ...
    print('Accuracy: {}/{} ({:.0f}%)'.format(correct2, len(test_loader.dataset), 100. * correct2 / len(test_loader.dataset)))
